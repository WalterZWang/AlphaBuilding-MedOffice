...
class ExperienceReplayBufferNPZ:
    def __init__(self, buffer_size):
        assert isinstance(buffer_size, int)
    ...
    def sample_order(self, start_index, end_index):
        """
        Get one non-random batch from experience replay
    ...
    def populate(self, file_path):
        """
        Populates samples in the file into the buffer
    ...
    def random(self):
        """
        Random shuffle the order of the elememts in the buffer
        """
        random.shuffle(self.buffer)

def unpack_batch_il(batch):
    states, actions = [], []
    for exp in batch:  # exp is 1-D numpy array
        state = exp[[0,3,1]+list(range(7,16))+[6]]  # add officeHour to the state, column 6 is office hour
        states.append(state)
        actions.append(exp[-19:])
