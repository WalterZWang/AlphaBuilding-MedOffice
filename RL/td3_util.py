...
class Agent():
    ...
    def learn(self):
        ...
        target_actions = target_actions + \
            T.clamp(T.tensor(np.random.normal(scale=self.noise_targetAct)), -0.5, 0.5)
        ...
        actor_q1_loss = self.critic_1.forward(state, self.actor.forward(state))  # can also use the mean 
